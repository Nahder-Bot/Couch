...
| HUMAN-VERIFY | Couch viz visual + iOS PWA UAT (Phase 14 / 14-04 Task 5 — must run before v34 production deploy; recap: hero icon renders, headline + sub-count, grid scales 2-10 cells with 5×2 phone wrap / 10×1 desktop, claim/vacate/already-claimed flows, Firestore round-trip, iOS PWA tap targets ≥44pt, prefers-reduced-motion respected) | .planning/phases/14-decision-ritual-core/14-04-SUMMARY.md |
...
| Deploy gate | `firebase deploy --only firestore:rules --project queuenight-84044` from sibling queuenight/ repo BEFORE v34 production deploy — Phase 14 / 14-04 Task 4 widened firestore.rules with a third UPDATE branch on /families/{code} for couchSeating writes; couch repo's `bash scripts/deploy.sh` only handles `--only hosting` (scripts/deploy.sh:142), so without this separate deploy live persistCouchSeating() writes will be denied. Recommended remediation (out of scope for 14-04): extend scripts/deploy.sh to add `--only hosting,firestore:rules` with mirror-check guard between the two repos. | .planning/phases/14-decision-ritual-core/14-04-SUMMARY.md "Deploy reminder" |
