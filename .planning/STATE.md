...
last_updated: "2026-04-30T03:25:04.077Z"
progress:
  total_phases: 19
  completed_phases: 15
  total_plans: 88
...
Phase: 15.6 (audit-trail-backfill-2) — EXECUTING
Plan: 3 of 3
...
| Pre-deploy gate | ✓ RESOLVED 2026-04-29 (Plan 18-04 Task 2 pre-flight) — TMDB_KEY was missing from queuenight/functions/.env; auto-fixed pre-deploy by appending `TMDB_KEY=2ec1f3699afc80f35392f5a674eb9da3` (same public-by-design value as couch js/constants.js). firebase v2 onSchedule deploy logged `Loaded environment variables from .env.` confirming pickup. Row retained for audit trail. | .planning/phases/18-availability-notifications/18-04-SUMMARY.md |
...
| Phase 18 progress | ✓ COMMITTED 2026-04-29 (Plan 18-03) — couch commits `c5fb713` (test) + `fac3d71` (chore) on `main`. scripts/smoke-availability.cjs (185 lines / 23 assertions / ~344ms live) regression-locks Plan 18-01's pure helpers via cross-repo require with graceful skip; scripts/deploy.sh §2.5 third smoke if-block + echo update; package.json adds smoke:availability + extends chained smoke to all 3 contracts. `npm run smoke` end-to-end verified ALL ASSERTIONS PASSED across position + tonight + availability. Wave 1 of Phase 18 closed. Next: Plan 18-04 cross-repo deploy ritual + UAT. |
| Phase 18 progress | ✓ DEPLOYED 2026-04-29 (Plan 18-04 — final wave) — couch commit `adcebfc` on `main` (`chore(18-04): bump sw.js CACHE to couch-v36-availability-notifs`). queuenight functions deploy 22:48Z (`providerRefreshTick(us-central1) Successful create operation` — NEW v2/scheduled/nodejs22/256MB; 25 other CFs Successful update); couch hosting deploy 22:50Z via `bash scripts/deploy.sh 36-availability-notifs` (43-passing rules suite + 3-contract smoke gate + sw.js bump no-op + mirror + Sentry DSN guard + firebase deploy --only hosting). Live curl checks: `couch-v36-availability-notifs` + HTTP 200 + Sentry DSN sentry.io/4511281871454208. Cross-repo ordering preserved (queuenight FIRST → couch SECOND per D-21 / RUNBOOK §H). UAT pending real-device verification — see 18-04-SUMMARY.md and the `/gsd-verify-work 18` workflow. | .planning/phases/18-availability-notifications/18-04-SUMMARY.md |
| HUMAN-VERIFY | Phase 18 UAT (4 scripts) — pending real-device verification post-2026-04-29 deploy. Scripts: (1) Settings UI 'Newly watchable' toggle parity in legacy + friendly-UI surfaces (default ON, hint verbatim); (2) Detail-modal manual refresh affordance — '↻ Refresh availability' button + 'Provider data via TMDB' footnote + flashToast 'Availability refreshed.' + lastProviderRefreshAt write; (3) Simulated CF tick via Firebase Console scheduler-page click (gcloud not installed locally) OR 24h soak — confirm push delivery for at least one queued title with provider change; (4) Opt-out smoke — toggle OFF → no push arrives. UAT 5 (quiet hours) optional. Resume signal: "uat passed" → trigger /gsd-verify-work 18. | .planning/phases/18-availability-notifications/18-04-SUMMARY.md |
| HUMAN-VERIFY (soak) | Phase 18 post-deploy 7-day Sentry soak — monitor [18/providerRefreshTick] error breadcrumbs. Persistent TMDB 429 spike past 1 day → reduce per-tick cap from 50. Also monitor 30-day usage analytics on titleAvailable opt-out rate (RESEARCH risk 2 — if opt-out >30%, revisit cadence/threshold per Deferred Ideas). | .planning/phases/18-availability-notifications/18-04-SUMMARY.md |
| Tooling gap | gcloud CLI not installed on local Windows machine — surfaced in Plan 18-04. Affects: (a) UAT 3 manual-trigger via `gcloud scheduler jobs run firebase-schedule-providerRefreshTick-us-central1` — workaround: Firebase Console scheduler-page click OR 24h Path B soak; (b) RUNBOOK §K Firestore daily export setup (already pending HUMAN-VERIFY). Recommended future install for full deploy/observability symmetry. | .planning/phases/18-availability-notifications/18-04-SUMMARY.md | .planning/phases/18-availability-notifications/18-03-SUMMARY.md |
