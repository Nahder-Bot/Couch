...
| 15. Tracking Layer | 0/8 | **PLANNED 2026-04-26** — 8 plans across 7 waves minted at /gsd-plan-phase 15 with researcher + planner + UI-spec + patterns coverage; 13 TRACK-15-* requirements minted in REQUIREMENTS.md; per-watching-group progress (tracks GROUPS, not just individuals — Couch's differentiator vs Trakt) + new-season air-date pushes + live-release scheduling. Plans: 15-01 (rules + indexes + tests, queuenight, W1) → 15-02 (tuple read/write helpers, couch, W2) ‖ 15-06 (CF live-release sweep + push category server gate, queuenight, W2) → 15-03 (D-01 watchparty-end auto-track stash + toggleMutedShow, couch, W3) → 15-04 (S2 detail-modal + S3 inline rename + S6 kill-switch + Phase 15 CSS namespace + post-session confirmation, couch, W4) → 15-05 (S1 Tonight tab pickup widget, couch, W5) → 15-07 (push triad client side + Trakt overlap S5 prompt, couch, W6) → 15-08 (close-out: REQUIREMENTS.md mint + sw.js CACHE bump v35.0-tracking-layer + changelog.html v35 + cross-repo deploy ritual checkpoint, couch, W7). **Originally tagged Phase 14.** | - |
