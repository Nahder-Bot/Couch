...
| 19. Kid Mode | 0/? | **SCOPED 2026-04-29, awaiting kickoff** — Cross-AI review 2026-04-28 (Gemini + Codex unanimously) flagged kid-mode / age-gating as a P0 gap for the "family" brand. Today js/app.js:5028 has per-member age-tier cap but no global "Kids on the couch tonight" toggle that filters Tonight to G/PG. Phase 19 adds: (a) one-tap "kid mode" toggle on the V5 roster surface (state-aware: amber when active, hidden when no kids in roster), (b) tier-cap filter applied to Tonight matches/considerable + Library + spin-pick + Flow A picker, (c) optional bedtime/runtime ceiling per kid mode session, (d) parent-approved pool override (parent OK on a normally-tier-blocked title applies for the kids-on-the-couch session). Out of scope: per-kid profile differentiation, hardware parental controls integration. | - |
...
*v1 milestone: Commercial Release (Phases 3-15.4 + 18 + scoped 15.3 / 16 / 17) — slug `v1-commercial-release`*
