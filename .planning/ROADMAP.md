...
| 15.1. Security Hardening | 0/3 | **SCOPED 2026-04-27** — Inserted post-Phase-15 audit. Closes 4 findings: F-OPEN-01 HIGH (`t.progress[X].source` forgeable — Trakt-overlap trust break) + WR-02 HIGH (regex injection via attacker-controlled `memberId`) + F-OPEN-02 MEDIUM (`tupleNames` not write-scoped to participants) + WR-03 MEDIUM (no rules-test coverage for `coWatchPromptDeclined`). Five locked items: per-member `progress` isolation via dotted-path writes; `memberId` regex anchor + `auth.uid` cross-validation; `tupleNames.<tk>` participant check (rule-side + client pencil-hide both); rules tests #33-#35; `setBy` UI surfacing. Scope locked in `.planning/todos/pending/2026-04-27-phase-15-security-hardening-pass.md`; evidence in `.planning/phases/15-tracking-layer/15-SECURITY.md` + `15-REVIEW.md`. **Inserted as decimal phase (mirrors 03.5 precedent), NOT a slot reassignment.** | - |
