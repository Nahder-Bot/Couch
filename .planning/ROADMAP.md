...
| 14. Decision Ritual Core | 8/9 | **IN PROGRESS** — 14-01 (already-watched filter, D-01 / DECI-14-01) + 14-02 (queue polish + Add-tab insertion + iOS DnD UAT, D-03 / DECI-14-03) + 14-06 (extend intents collection + CFs, D-09 / DECI-14-09) shipped 2026-04-25; 14-03 (tier aggregators T1/T2/T3 + T3 visibility resolver, D-02 / DECI-14-02) + 14-04 (Couch viz hero-icon + avatar-grid per sketch 001 winner P, D-06 / DECI-14-06) + 14-05 (tile redesign + Vote-mode preservation + carry-over UAT bug closure, D-04 + D-05 / DECI-14-04 + DECI-14-05) + 14-07 (Flow A group rank-pick + push-confirm UI, D-07 / DECI-14-07) + 14-08 (Flow B solo-nominate + counter-time + auto-convert + all-No edge, D-08 / DECI-14-08) shipped 2026-04-26. **14-02 Task 3 iOS Safari touch-DnD UAT + 14-04 Task 5 visual + iOS PWA Couch viz UAT + 14-07 Task 5 multi-device Flow A UAT + 14-08 Task 5 multi-device Flow B UAT all DEFERRED** to batch UAT pass — all must run before v34 production deploy (tracked in STATE.md follow-ups; 14-02 has documented Sortable.js fallback if touch-DnD fails; 14-07 + 14-08 multi-device UAT items are bundled conceptually since same staging deploy + 2-device session covers both flows). **14-05 Task 5 (carry-over UAT bug) CLOSED — NOT DEFERRED** — by pre-existing CSS fix at css/app.css:1540 (`.detail-close{position:fixed}`); soft iOS Safari render confirmation absorbed into the existing 14-02 iOS Safari follow-up entry as non-blocking. **DEPLOY GATE for v34** added by 14-04 Task 4: a separate `firebase deploy --only firestore:rules --project queuenight-84044` from sibling queuenight/ repo is REQUIRED before v34 production deploy because `bash scripts/deploy.sh` only handles `--only hosting` (scripts/deploy.sh:142) — without it live couchSeating writes from persistCouchSeating() will be denied. **Only 14-09 remains** — onboarding tooltips + 5 empty states + 7 push categories + sw.js v34 CACHE bump. **NOTE for 14-09 owner:** the 7 new eventTypes (`flowAPick`, `flowAVoteOnPick`, `flowARejectMajority`, `flowBNominate`, `flowBCounterTime`, `flowBConvert`, `intentExpiring`) are wired in queuenight/functions/index.js (uncommitted — queuenight is not a git repo) AND in BOTH 14-07's client surface (Flow A) and 14-08's client surface (Flow B) but NOT yet present in `NOTIFICATION_DEFAULTS` / `DEFAULT_NOTIFICATION_PREFS` / `NOTIFICATION_EVENT_LABELS` — deploy 14-06 CF first; user toggles silently default-on until 14-09 ships. **Originally tagged Phase 13 — see Phase-slot history below.** | 14-07 + 14-08 complete 2026-04-26 |
