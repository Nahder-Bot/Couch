...
| 18. Availability Notifications | 4/4 | **DEPLOYED 2026-04-29 — UAT PENDING** — All 4 plans deployed cross-repo to production: queuenight functions at 22:48Z (`providerRefreshTick(us-central1) Successful create operation` — NEW v2/scheduled/nodejs22/256MB; 25 other CFs Successful update; TMDB_KEY auto-fixed pre-deploy) → couch hosting at 22:50Z via `bash scripts/deploy.sh 36-availability-notifs` (43-passing rules suite + 3-contract smoke gate + sw.js bump + mirror + Sentry DSN guard + firebase deploy --only hosting). Live curl checks: `couch-v36-availability-notifs` + HTTP 200 + Sentry DSN sentry.io/4511281871454208. Cross-repo ordering preserved (queuenight FIRST → couch SECOND per D-21 / RUNBOOK §H). REQ-18-09 + REQ-18-10 closed; REQ-18-01..REQ-18-08 await Task 4 UAT (4 scripts: Settings UI parity + manual refresh affordance + simulated CF tick + opt-out). UAT instructions surfaced to user via checkpoint:human-verify; resume signal `uat passed` → trigger /gsd-verify-work 18. Earlier Plans 18-01 + 18-02 + 18-03 complete. 18-01: queuenight providerRefreshTick scheduled CF + titleAvailable in NOTIFICATION_DEFAULTS + 3 module-exported pure helpers (addedBrandsFor + buildPushBody + normalizeProviderName) + server-side fetchTmdbProvidersServerSide at queuenight commit `d622dc6` (local-only). 18-02: couch js/app.js DR-3 client mirror complete — titleAvailable added to DEFAULT_NOTIFICATION_PREFS + NOTIFICATION_EVENT_LABELS + NOTIF_UI_TO_SERVER_KEY + NOTIF_UI_LABELS + NOTIF_UI_DEFAULTS (5 maps, places 2-6 of 6); detail-modal manual-refresh affordance polished — button label `↻ Refresh availability` (D-17), `lastProviderRefreshAt: Date.now()` write so CF round-robin stays accurate (D-18), `flashToast('Availability refreshed.')` success + `flashToast('Refresh failed. Try again.')` failure replacing alert(), `'Provider data via TMDB'` italic attribution footnote (D-16). 3 commits: `5a86045` + `7f04d2f` + `416ab32`. 18-03: scripts/smoke-availability.cjs contract suite (185 lines / 23 assertions / ~344ms) regression-locks D-07 + D-14 + D-15 contracts via cross-repo `require('../../../queuenight/functions/index.js')` with fs.existsSync graceful skip; scripts/deploy.sh §2.5 gains third smoke if-block + echo update; package.json adds `smoke:availability` and chains all 3 in `smoke`. 2 commits: `c5fb713` + `fac3d71`. Wave 1 closed. Wave 2: 18-04 (sw.js CACHE bump + cross-repo deploy ritual + UAT). Cross-repo: queuenight FIRST, then couch (D-21). | Plans 1+2+3 of 4 done 2026-04-29 |
