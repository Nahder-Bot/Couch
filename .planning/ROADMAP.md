...
| 14. Decision Ritual Core | 3/9 | **IN PROGRESS** — 14-01 (already-watched filter, D-01 / DECI-14-01) + 14-02 (queue polish + Add-tab insertion + iOS DnD UAT, D-03 / DECI-14-03) + 14-06 (extend intents collection + CFs, D-09 / DECI-14-09) shipped 2026-04-25. **14-02 Task 3 iOS Safari touch-DnD UAT DEFERRED** to batch UAT pass — pre-existing drag-reorder behavior unchanged by this plan; UAT must run before v34 production deploy (tracked in STATE.md follow-ups; documented Sortable.js fallback if it fails). Remaining W1: 14-04 SVG couch viz, 14-05 tile redesign. W2: 14-03 tier aggregators, 14-08 Flow B nominate. W3: 14-07 Flow A rank-pick. W4: 14-09 onboarding tooltips + 5 empty states + 7 push categories + sw.js v34 CACHE bump. **NOTE for 14-09 owner:** the 7 new eventTypes (`flowAPick`, `flowAVoteOnPick`, `flowARejectMajority`, `flowBNominate`, `flowBCounterTime`, `flowBConvert`, `intentExpiring`) are wired in queuenight/functions/index.js (uncommitted — queuenight is not a git repo) but NOT yet present in `NOTIFICATION_DEFAULTS` / `DEFAULT_NOTIFICATION_PREFS` / `NOTIFICATION_EVENT_LABELS` — deploy 14-06 CF first; user toggles silently default-on until 14-09 ships. **Originally tagged Phase 13 — see Phase-slot history below.** | 14-02 complete 2026-04-25 |
