...
**Plans**: TBD — kickoff via `/gsd-discuss-phase 24` (default chain: discuss → plan → execute, per project GSD workflow pattern).
...
| 24. Native video player (iframe + HTML5) | 0/? | **SCOPED, awaiting kickoff** — promoted to ROADMAP 2026-04-30 from v2 milestone plan locked in `22-SUMMARY.md` lines 100-115. Iframe wrapper (YouTube / Twitch / Vimeo) + HTML5 `<video>` for self-hosted MP4 / M3U8 / Plex / Jellyfin URLs. `Player.currentTime` broadcast to Firestore so Phase 26 reactions can anchor. Merges old Phase 24 + 25 into single deploy. **Hard ordering constraint: must precede Phase 26** (currentTime broadcast is the data source for runtime-indexed reactions). Full v2 plan + cross-phase dependencies in `seeds/v2-watchparty-sports-milestone.md`. | - |
