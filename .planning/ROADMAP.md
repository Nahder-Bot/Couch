...
| 18. Availability Notifications | 1/4 | **EXECUTING 2026-04-29** — Plan 18-01 complete: queuenight providerRefreshTick scheduled CF + titleAvailable in NOTIFICATION_DEFAULTS + 3 module-exported pure helpers (addedBrandsFor + buildPushBody + normalizeProviderName) + server-side fetchTmdbProvidersServerSide all landed at queuenight commit `d622dc6` (local-only, deploy gated to Plan 18-04). Daily us-central1 256MiB 540s schedule; 50-title-per-tick cap; round-robin oldest-first via lastProviderRefreshAt with cold-start exemption; setBy:'system' attribution marker on writebacks; sendToMembers eventType-gated push fan-out. Wave 1 continues: 18-02 (couch client mirror in 5 places + manual refresh affordance) + 18-03 (smoke-availability.cjs contract test). Wave 2: 18-04 (cross-repo deploy ritual + UAT). Cross-repo: queuenight FIRST, then couch (D-21). | Plan 1 of 4 done 2026-04-29 |
