...
- **Mark (canonical brand-storytelling icon):** leather-cushion **capital "C" with a glowing TV nested in its opening** — same leather material, stitching, lighting, backdrop as the wordmark, plus an amber-glowing screen positioned where the C "faces" the right edge. The composition tells the product story in one symbol: a couch facing a TV (movie night). Master at `brand/mark-master.png` (1024×1024 **square**, full-canvas-fill, NO baked-in rounded corners — iOS / Android apply their own corner masks at render time). Downscaled set generated via sharp pipeline: `mark-{16,29,32,40,48,58,60,76,80,87,96,120,128,144,152,167,180,192,384,512,1024}.png`. Apple-touch-icon at 128/144/152/180.
- **Maskable variants** ship with the W3C 40% safe-zone (the C+TV composition is centered with ~15% padding so adaptive-icon circular/rounded-square cropping never clips the C silhouette; the TV may be partially clipped on circular masks but the C reads cleanly). Installed PWA on Android + iOS home-screen get the full mark.
- **Notification icon (Android):** flat white silhouette of the C on transparent background, ≤96×96, derived from the C silhouette only (NO TV detail — Android's notification strip can't render photorealistic textures or color, so the TV is dropped at this size). Master at `brand/notification-mark.png`.
- **Small-size behavior:** below ~120px the TV detail collapses to a faint warm glow on the right side of the C — accept this; the C silhouette stays clearly readable and the warm glow becomes a brand element rather than a literal screen. Tested down to 16×16 (favicon).
