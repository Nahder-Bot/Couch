...
### Phase 15.3 (canonical SVG logo) soft-promotion
- **D-09:** Phase 15.3 (DESIGN-01: canonical SVG logo + wordmark sources) was deferred per LAUNCH-REVIEW §6 P2. **App Store assets audit (2026-05-06) reverses this judgment.** Without canonical SVG sources, generating the Apple icon matrix (1024×1024 marketing icon + 87×87 + 58×58 + 29×29 + 120×120 + 80×80 + 40×40 + 60×60 + 20×20 in @1x/@2x/@3x) requires re-vectorization from existing PNG (lossy) or re-design (slow). **Recommended:** schedule Phase 15.3 as Phase 17 Wave 0 dependency. ~1-2 days SVG production. OPTIONAL: rasterize from `mark-512.png` to `mark-1024.png` via sharp upscale (lossy; reviewer-acceptable but visible at 100% zoom). Decision deferred to user at /gsd-discuss-phase 17.
