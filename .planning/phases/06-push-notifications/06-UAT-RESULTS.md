...
|---|---|---|---|---|
| 1 | **iOS standalone PWA — watchparty-starting (flagship)** — RSVP to a watchparty scheduled 2min out from a second device, wait for status→active, confirm push arrives within ~5s | PENDING-HARDWARE | — | Requires iOS 16.4+ home-screen PWA. If fail: check Safari Web Inspector console on iPhone; verify `navigator.serviceWorker.ready` + `pushManager.getSubscription()` both resolve. |
| 2 | **Self-echo guard** — actor does NOT receive push for their own action across 3 event types (schedule watchparty, approve title, receive invite you sent to yourself) | PENDING | — | Tests PUSH-05. Relies on excludeUid wiring in 06-02. |
